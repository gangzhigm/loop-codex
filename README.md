...
> AI 角色以 `AGENTS.md`、对应 `prompts/*.md`、受控入口返回值和目标项目规则为准。
...
py -3 .\scripts\roles\supervisor\control.py health
py -3 .\scripts\roles\supervisor\control.py serve
```
...
| Dashboard 健康与前台服务 | `scripts/roles/supervisor/control.py` |
...
| `scripts/roles/` | Planner、Worker、Supervisor、Dispatcher 和 Runner 入口 |
| `scripts/roles/supervisor/health_run.py` | Dashboard 健康检查内部实现 |
| `scripts/installers/install_health_task.ps1` | Windows 健康任务安装器 |
| `scripts/tests/` | Python 回归测试 |
| `frontend/` | Dashboard 前端资源 |
...
3. 根据失败角色读取对应 `prompts/*.md` 和 `scripts/README.md` 的职责导航。
