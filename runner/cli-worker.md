...
3. 发布、Git 提交、外部消息、凭据访问，以及删除 Git 已跟踪、attempt 前已存在或归属不明的文件，只有在当前任务文本含有 `APPROVED_ACTIONS: delete,publish,git_commit,external_message,credential_access` 中对应动作时才允许。缺少批准时返回 `WAITING_HUMAN`，不要尝试执行。当前 attempt 的命令在任务登记项目内新生成的普通临时文件，可作为任务内资源清理自行删除而无需 `delete` 批准，但必须同时满足：执行前 Git 状态中不存在、执行后仍未被 Git 跟踪、能由具体命令和时间证明归属当前 attempt、不是目录或符号链接/重解析点、不是源码/配置/凭据/用户数据，并使用逐个精确路径删除。禁止通配符、递归删除、宽泛文件名匹配或扩大到其他项目；删除后重新检查 Git 状态并在 verification 中记录。任一条件无法确认时不得删除，返回 `WAITING_HUMAN`。终止当前 attempt 为完成任务而直接启动的业务命令及其子进程，同样属于任务内资源清理；但必须能用当前工具调用句柄、精确 PID 或明确的父子进程关系证明归属。不得按模糊进程名、端口猜测或宽泛匹配终止进程，也不得终止 attempt 前已存在、归属不明、其他任务、共享服务或开发服务器的进程。
   ...
   对于已证明满足前述普通临时文件全部条件、仅为普通测试日志或可丢弃测试临时文件，且残留不会影响业务实现、验证结果、用户数据或后续执行的文件，如果宿主策略或工具权限拒绝该逐个精确路径删除，只记录精确路径、生成命令和拒绝原因到 verification，不得仅因此返回 `WAITING_HUMAN`。此例外不放行 attempt 前已存在、归属不明、已跟踪、源码、配置、凭据、数据库、用户数据、目录、符号链接/重解析点或可能影响其他任务的文件；任一条件无法确认时仍不得删除并返回 `WAITING_HUMAN`。

   唯一允许自动恢复的已跟踪范围外文件是当前 attempt 的 Python 命令生成或改写的 CPython 字节码缓存，路径必须位于同一登记项目的 `__pycache__` 目录且文件名以 `.pyc` 结尾。恢复前必须同时证明：attempt 首次编辑或测试前保存的完整 Git 状态中该精确路径没有工作区或暂存改动；当前差异由本 attempt 记录的具体 Python/测试命令和时间产生；文件不是符号链接或重解析点；Git 索引内容与 attempt 前内容一致。满足全部条件时，只能使用逐个精确路径把工作区文件恢复到索引中的 attempt 前内容，不得删除文件、修改索引、使用通配符或递归操作，也不要求该缓存位于 task.scope。恢复后必须再次核对该路径无工作区和暂存差异，并把生成命令、精确路径和恢复结果写入 verification。attempt 前已有差异、缺少完整基线、无法证明归属、索引已变化或文件不符合该严格形态时，仍必须 `WAITING_HUMAN`。该例外不适用于源码、配置、资源、数据库、文档或其他已跟踪文件。
