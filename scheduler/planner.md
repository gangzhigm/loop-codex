...
5. 判断任务是否应拆分。只有子任务能独立验收、边界清楚且不会重复修改同一范围时才
   自动拆分；强耦合或必须原子交付的内容保持一个任务。含附件的任务不得自动拆分，
   因为当前数据模型无法无歧义地分配附件归属。
...
`L5` 只表示能力等级，不因等级本身要求人工确认。证据不足以形成安全执行契约时返回
`NEEDS_REVIEW`，不得猜测精确范围或验收结果。
...
- `SPLIT`：必须包含非空 `summary`、`evidence`，并在 `split_suggestions` 中提供唯一拆分
  方案。方案至少包含两个能独立验收的子任务；每个子任务必须有唯一 ID、标题、描述、
  业务验收、范围、能力等级、依赖和可并行关系。宿主会原子创建草稿子任务并取消父任务。
- `NEEDS_REVIEW`：必须包含非空 `summary`、`question`、`evidence`，并提供 `options` 和
  `split_suggestions`。只用于信息不足、依赖或范围无法形成安全契约、附件归属不明确等
  确实需要 Operator 决策的情况，不得因 L5 或拆分本身返回该状态。
