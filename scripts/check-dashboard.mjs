...
assert(html.includes('const SECRET_API_ENDPOINT = "/api/secrets"'), "缺少 Provider Secret API 地址");
assert(html.includes('fetch(SECRET_API_ENDPOINT'), "密钥管理视图未通过本机 Secret API 工作");
assert(html.includes('"X-CSRF-Token": secretCsrfToken'), "Secret 写请求未携带 CSRF token");
assert(html.includes('request_id: crypto.randomUUID()'), "Secret 写请求未生成一次性请求 ID");
assert(!/\b(?:localStorage|sessionStorage|indexedDB)\b/.test(html), "Dashboard 不得把 Secret 写入浏览器持久化存储");
assert(!html.includes("api.deepseek.com"), "前端不得直接请求 DeepSeek");
assert(/data-secret-candidate type="password" autocomplete="off"/.test(html), "Secret 输入未使用非预填 password 控件");
assert(!/data-secret-candidate[^>]*\svalue=/.test(html), "Secret 输入不得包含预填 value");
assert(html.includes('candidateInput.value = "";'), "Secret 提交前未立即清空 DOM 输入");
assert(html.includes('candidate = "";'), "Secret 提交后未清空临时候选值");
assert(html.includes('body.secret = "";'), "Secret 请求对象未及时清空提交字段");
assert(html.includes('data-toggle-secret aria-controls='), "Secret 输入缺少显示/隐藏控件");
assert(html.includes('data-secret-action="verify-local"'), "密钥管理视图缺少本地验证操作");
assert(html.includes('data-secret-action="verify-connect"'), "密钥管理视图缺少连接验证操作");
assert(html.includes('data-secret-action="delete"'), "密钥管理视图缺少删除操作");
assert(html.includes('连接验证可能产生一次 Provider 调用。确认继续？'), "连接验证缺少费用/调用确认");
assert(html.includes('确认替换 ${providerDisplayName(providerId)} 的现有密钥？'), "替换 Secret 缺少明确确认");
assert(html.includes('确认删除 ${providerDisplayName(providerId)} 的密钥？此操作不能撤销。'), "删除 Secret 缺少明确确认");
for (const secretStatus of ["未配置", "已配置 · 未验证", "有效", "无效", "存储不可用"]) {
  assert(html.includes(secretStatus), `密钥管理视图缺少 ${secretStatus} 状态`);
}
assert(html.includes('role="dialog" aria-modal="true"'), "Provider 密钥抽屉缺少模态对话框语义");
assert(html.includes('id="secretApiNotice" class="secret-api-notice" role="status" aria-live="polite"'), "Secret API 结果缺少屏幕阅读器播报区域");
assert(html.includes("function trapSettingsFocus(event)"), "Provider 密钥抽屉缺少键盘焦点约束");
assert(html.includes('for="${escapeHtml(inputId)}"'), "Secret password 控件缺少显式 label");
assert(/\.settings-drawer\s*\{[\s\S]*?width:\s*min\(440px, 100%\);/.test(html), "设置抽屉宽度未排除移动端滚动条造成的 100vw 溢出");
...
assert(html.includes('data-recovery-action="failed"'), "Dashboard 缺少安全恢复失败处置入口");
assert(html.includes('data-recovery-action="wait"'), "Dashboard 缺少继续等待入口");
...
assert(html.includes('<th class="task-action-heading" style="width: 10%">操作</th>'), "任务表格缺少稳定宽度的操作列");
assert(html.includes('<td class="task-action-cell" data-label="操作">${renderTaskAction(task)}</td>'), "任务行缺少固定操作单元格");
assert(/\.task-action-heading\s*,\s*\.task-action-cell\s*\{[\s\S]*?padding-inline:\s*8px 24px;[\s\S]*?text-align:\s*center;/.test(html), "操作列表头与内容未使用统一右侧留白");
assert(/td\.task-action-cell\s*\{[\s\S]*?padding:\s*0 12px 0 0;/.test(html), "窄屏操作单元格未保留右侧留白");
...
assert(html.includes('const levelProfiles = { L1: "routine", L2: "standard", L3: "advanced", L4: "deep", L5: "complex" };'), "调度容量缺少固定 L1-L5 映射");
assert(html.includes('profile_parallel_limits'), "调度容量未读取档位并发上限");
assert(html.includes('global_max_active_executions'), "调度矩阵未展示全局容量");
assert(html.includes('agents.filter((agent) => agent.capability_level === level).length'), "活动 execution 未按能力等级跨环境统计");
assert(html.includes('title="${escapeHtml(profile)}"'), "完整档位名称未保留在 tooltip 中");
assert(html.includes('class="profile-level-label"'), "调度矩阵缺少能力等级标题");
