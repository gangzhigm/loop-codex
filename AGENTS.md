...
6. 正常流程通过 UTF-8 stdin 提交 Worker 结构化结果，不创建持久化 report。

## 权威数据边界

- `data/loop-agent.sqlite3`：任务、历史 Planner 预检及执行一致性数据的唯一事实来源；禁止重建运行时 `TASKS.json` 或 `INBOX.json`。
...
- Planner Worker：`worker/planner_codex_runtime.py` 使用固定 Planner 能力档位领取一个 `PLANNER/QUEUED` execution，只读判断最终能力、范围、冲突和是否拆分，并通过预检控制协议写回；安全拆分由宿主原子创建草稿子任务并取消父任务。
