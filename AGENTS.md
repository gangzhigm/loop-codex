...
3. 模型映射、自动化周期、任务执行参数、项目默认优先级和服务部署参数只读取 `config/initialization.json`，不得保存到 SQLite；SQLite 只在任务行保存 `execution_profile`。
...
7. Worker 自动化的权威提示词是 `prompts/worker.md`；每次必须带自身 `execution_profile` 只调用一次 `claim`，只处理领取结果中的一个任务，不创建或等待其他任务，也不读取或修改 Codex 自动化状态。
...
15. 所有档位共享最多 6 个活动 execution，并同时受各档位并发上限约束；优先级、执行档位、依赖和 scope 锁是四个独立维度。
